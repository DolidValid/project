...
const ImportBatch = ({ type, fileId, executionDate }) => {
  ...
  const executionDateFromParent =
    executionDate || location.state?.executionDate || new Date().toISOString();
  ...
  const handleConfirmRun = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const successStatuses = parsedPayloads.map((_, i) => ({
          row: i + 1,
          status: "success",
        }));
        ...
        setError(
          `Upload Failed: ${errData.error || errData.message || "Unknown error"}`,
        );
      }
    } catch (err) {
      ...
      <div className="import-card">
        ...
        <div className="text-center mb-4">
          ...
          <div className="file-info-badge">
            <LuSearch size={16} className="me-2" /> File ID:{" "}
            <strong>{phoneFromInfoFile || "Manual Input"}</strong>
          </div>
        </div>
          ...
          <div className="d-flex gap-4 align-items-center mt-2">
            ...
            {isAllUploaded && (
              <button
                ...
                onClick={viewResults}
              >
                View Batch Results <LuArrowRight size={18} />
              </button>
            )}
          </div>
        </div>
